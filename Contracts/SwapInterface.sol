...
contract ExchangeInterface is Ownable {
    ...
    function addLiquidity(
        ...
        require(
            IERC20(_tokenA).approve(address(router), _tokenAAmount),
            "_tokenA approval issue"
        );
        require(
            IERC20(_tokenB).approve(address(router), _tokenBAmount),
            "_tokenB approval issue"
        );
        router.addLiquidity(
            address(_tokenA),
            address(_tokenB),
    ...
    function swapTokensForExactTokens(
        ...
        address _tokenB,
        uint256 _tokenBAmount,
        uint256 _tokenAAmountMax
    ) private {
        address[] memory path = new address[](2);
        path[0] = address(_tokenA);
        path[1] = address(_tokenB);
        require(
            IERC20(_tokenA).approve(address(router), _tokenAAmountMax),
            "_tokenA approval issue"
        );
